        ...
        <DialogContent className="fixed inset-0 z-[100] flex h-screen w-screen max-w-none flex-col items-center justify-center border-0 bg-transparent p-0 shadow-none sm:rounded-none">
          <Card className="h-full w-full flex flex-col items-center justify-center bg-gradient-to-br from-background via-accent/10 to-primary/10 p-6 overflow-auto">
            <Button variant="ghost" size="icon" onClick={onClose} className="absolute top-6 right-6 text-foreground/70 hover:text-foreground">
              <Minimize2 size={28} />
              <span className="sr-only">Close Fullscreen Player</span>
            </Button>
            ...
            <div className="w-full max-w-md space-y-4">
              ...
              <div className="flex items-center justify-center gap-4">
                ...
                <Button variant="ghost" size="icon" onClick={prevTrack} className="text-foreground/70 hover:text-foreground" aria-label="Previous track">
                  <SkipBack size={28} />
                </Button>
                <Button variant="default" size="icon" onClick={togglePlayPause} className="h-16 w-16 rounded-full bg-primary text-primary-foreground hover:bg-primary/90" aria-label={isPlaying ? "Pause" : "Play"}>
                  {isPlaying ? <Pause size={36} /> : <Play size={36} className="ml-1"/>}
                </Button>
                <Button variant="ghost" size="icon" onClick={nextTrack} className="text-foreground/70 hover:text-foreground" aria-label="Next track">
                  <SkipForward size={28} />
                </Button>
                <Button variant="ghost" size="icon" className="text-foreground/70 hover:text-foreground">
                  <Repeat size={20} />
              ...
              <div className="flex items-center justify-center gap-2 pt-4">
                <Button variant="ghost" size="icon" onClick={() => setVolume(volume > 0 ? 0 : 0.5)} className="text-foreground/70 hover:text-foreground" aria-label={volume > 0 ? "Mute" : "Unmute"}>
